...
obj_cv = function(parms, #parameters for model implemented between transfers (first because optim)
  ...
  for(i.compare in 1:nrow(dat.real)){
    ...
    err = (dat.real[i.compare,-1]+1)/scale.vec -
      (out$transf.pred[out$transf.pred[,1]==cur.transf,-1]+1)/scale.vec
    SS=SS+sum(err^2)
  }
  return(SS)
}
